...
public class UpLoadJsonAction {
	...
	public JSONObject upLoadImage(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			System.out.println("文件路径:" + pathTemp[0]);
			System.out.println("文件路径:" + pathTemp[1]);
			String saveUrl = pathTemp[0];

			// 文件保存目录路径
			// 这个地方改在虚拟目录，如果使用系统目录直接放开即可
			// String savePath =
			// pageContext.getServletContext().getRealPath("/") +
			// "kindeditor_attachment/";
			// 文件保存目录URL
			// 这个地方改在虚拟目录，如果使用系统目录直接放开即可
			...
			while (item.hasNext()) {
				...
				try {
					File uploadedFile = new File(pathTemp[0], newFileName);
					ByteStreams.copy(file.getInputStream(), new FileOutputStream(uploadedFile));
				} catch (Exception e) {
				}
				obj.put("error", 0);
				obj.put("url", RuoYiConfig.getSystemUrlAddress() + pathTemp[1] + newFileName);
				return obj;
			}
